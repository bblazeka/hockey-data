...
namespace HockeyDb.Services
{
    public class TeamService : DatabaseService
    {
        ...
        public int UpdateLeague(int teamId, string teamName, object league, int season)
        {
            ...
            "values (@TeamId, @LeagueId, @SeasonId)", new { TeamId = teamId, ((LeagueViewModel)league).LeagueId, SeasonId = season });
                    return affectedRows;
                }
        ...
        public List<PlayerViewModel> GetPlayers(object teamObj, string season)
        {
            ...
            using (SqlConnection connection = new SqlConnection(m_builder.ConnectionString))
            {
                var res = connection.Query<PlayerViewModel, PlayerSeasonViewModel, TeamViewModel, PlayerViewModel>(
